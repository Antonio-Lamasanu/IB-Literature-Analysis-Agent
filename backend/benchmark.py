...
def run_benchmark(config_path: Path) -> None:
    ...
    n_ctx: int = int(config.get("n_ctx", 2048))
    ...
    log.info("n_ctx=%d  n_threads=%d  max_tokens=%d  temperature=%s",
             n_ctx, n_threads, max_tokens, temperature)
    ...
    for model_path in model_paths:
        ...
        for prompt_format in PROMPT_FORMATS:
            ...
            # One service instance per (model, format) pair — avoids cross-format state
...
def _write_report(
    ...
    for model_name in models_seen:
        ...
        for fmt in PROMPT_FORMATS:
            fmt_records = [r for r in records if r["model"] == model_name and r["prompt_format"] == fmt]
            if not fmt_records:
                continue
            ...
            for r in sorted(fmt_records, key=lambda x: x["question_index"]):
        ...
        # Per-format timing summary
        lines.append("  Timing summary (inference seconds):")
        lines.append(f"  {'Format':<18} {'Q1':>6} {'Q2':>6} {'Q3':>6} {'Q4':>6} {'Q5':>6} {'Total':>8}")
        lines.append(f"  {'-'*18} {'---':>6} {'---':>6} {'---':>6} {'---':>6} {'---':>6} {'-----':>8}")
        for fmt in PROMPT_FORMATS:
            fmt_records = sorted(
                [r for r in records if r["model"] == model_name and r["prompt_format"] == fmt],
                key=lambda x: x["question_index"],
            )
            if not fmt_records:
                continue
            times = [r["inference_time_s"] for r in fmt_records]
