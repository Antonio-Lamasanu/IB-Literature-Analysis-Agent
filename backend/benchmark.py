                        ...
                        if test_context_length:
                            ...
                            for set_idx, stress_set in enumerate(context_stress_sets):
                                ...
                                for q_idx, question in enumerate(set_questions):
                                    ...
                                    rag_history_window = 10 if prompt_format in ("rag", "rag_raw") else None
...
def _write_report(
    ...
    # Collect unique values preserving first-seen order (from standard records)
    ...
    if stress_records:
        ...
        stress_n_ctx_seen: list[int] = []
        for r in stress_records:
            if r["n_ctx"] not in stress_n_ctx_seen:
                stress_n_ctx_seen.append(r["n_ctx"])

        # Collect stress set names in the order they appear
                        ...
                        for fmt in PROMPT_FORMATS:
                            cell = sorted(
                                [
                                    ...
                                    if r.get("question_set_name") == set_name and r["prompt_format"] == fmt
                                ],
                                key=lambda x: x["question_index"],
                            )
                            if not cell:
                            ...
                            lines.append(f"    Set: {set_name} ({len(cell)} turns) | format: {fmt}")

                            if fmt in ("rag", "rag_raw"):
    ...
    # Full answers section (standard mode)
    if standard_records:
        lines.append("=" * 80)
    ...
    # Full answers section (stress mode)
    if stress_records:
        lines.append("=" * 80)
        ...
        stress_n_ctx_seen_full: list[int] = []
        for r in stress_records:
            if r["n_ctx"] not in stress_n_ctx_seen_full:
                stress_n_ctx_seen_full.append(r["n_ctx"])
                        ...
                        for fmt in PROMPT_FORMATS:
                            cell_records = sorted(
                                [
                                    r for r in doc_stress
                                    if r["model"] == model_name and r["n_ctx"] == n_ctx
                                    and r.get("question_set_name") == set_name and r["prompt_format"] == fmt
                                ],
                                key=lambda x: x["question_index"],
                            )
                            if not cell_records:
                            ...
                            lines.append(f"--- {model_name} | n_ctx={n_ctx} | set={set_name} | {fmt} ---")
                            for r in cell_records:
                                q_num = r["question_index"]
                                tlabel = _trimmed_label(r)
                                dropped = r.get("history_turns_dropped", 0)
